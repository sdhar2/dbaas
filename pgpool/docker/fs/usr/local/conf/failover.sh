...
## in some situations the new master host name comes in as blank
## if so, then try to get the real host name from pg.conf artifact

if [ "$new_master_host_name" == "" ]; then
  new_master_host_name=`cat /tmp/pg.conf | grep -oP '(?<=database/postgres/postgres).*?(?=}|$)' |cut -d" " -f2 | grep -v $failed_host_name | head -1`
  if [ "$new_master_host_name" == "" ]; then
    echo "new master host name is blank in both etcd and this script. Exiting."
    exit 1
  else
    echo "new master host name is blank, using "$new_master_host_name" instead"
  fi
fi
